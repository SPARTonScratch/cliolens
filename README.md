...
| `--show-binary` | off | Show a placeholder notice for binary files in File Contents (they always appear in the tree and counts) |
...
fallback authority. They **always** appear in the directory tree and count
toward included files, but their contents are never dumped. Use `--show-binary`
to add a placeholder notice for each one in the File Contents section.
...
- With `--show-binary`, binaries appear as
