...
ClioLens packs a project directory into a single markdown *context dump* —
a visual directory tree, the full text of every relevant file, and metadata
with token estimates — designed to be attached to free web-based AI models
(Qwen, Kimi, ChatGPT, Claude…) for whole-codebase analysis.

No API keys. No local GPU. No IDE extensions. No telemetry. Read-only.
...
```powershell
# Dump the current directory to a file, then attach it to your model of choice
